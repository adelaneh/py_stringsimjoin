...
def jaccard_join(ltable, rtable,
    ...
    if output_file != None and scratch_dir != None and flush_after > 0:
        write_output_to_file = True



    if not write_output_to_file:
        return _jaccard_join_output_table(ltable, rtable,
        ...
        _jaccard_join_output_file(ltable, rtable,
                                  ltable_array, rtable_array,
                                  l_key_attr, r_key_attr,
                                  l_join_attr, r_join_attr,
                                  tokenizer,
                                  ...
                                  revert_tokenizer_return_set_flag,
                                  n_jobs, show_progress,
                                  output_file, scratch_dir, flush_after
                                  )
        return True



from py_stringsimjoin.utils.generic_helper \
    import get_output_header_from_tables, write_file_with_id, get_temp_filenames, \
    merge_outputs ,remove_files, flush_to_file, should_flush
import math
def _jaccard_join_output_file(ltable, rtable,
                              ltable_array, rtable_array,
                              l_key_attr, r_key_attr,
                              l_join_attr, r_join_attr,
                              tokenizer,
    ...
    pd.DataFrame(columns=output_header).to_csv(output_file, index=False, mode='w+')
    ...
    if n_jobs <= 1:
        ...
        _flush_after = math.ceil(rtable_array, n_jobs)
        ...
        results = Parallel(n_jobs=n_jobs)(delayed(set_sim_join)(
            ...
            _intermediate_output_files[job_index],
            _flush_after) for job_index in range(n_jobs))


        merge_outputs(_intermediate_output_files, output_file)
        remove_files(_intermediate_output_files)

        output_table = pd.concat(results)

        if should_flush(output_table, flush_after):
            flush_to_file(output_table, output_file)
...
def _jaccard_join_output_table(ltable, rtable, ltable_array, rtable_array,
